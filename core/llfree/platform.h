...
///
/// This macro loads the value at `atom_ptr`, stores its llfree_result in
/// `old_val` and then executes the `fn` function with a pointer to the loaded
/// value, which should be modified and is then stored atomically with CAS. The
/// function `fn` can take any number of extra parameters, that are passed
/// directly into it.
///

...
static int ena_com_init_comp_ctxt(struct ena_com_admin_queue *admin_queue)
{
	...
	admin_queue->comp_ctx = reinterpret_cast<decltype(admin_queue->comp_ctx)>(ENA_MEM_ALLOC(admin_queue->q_dmadev, size));
	if (unlikely(!admin_queue->comp_ctx)) {
		ena_trc_err(ena_dev, "Memory allocation failed\n");
		return ENA_COM_NO_MEM;
	}
			      ...
			      struct ena_com_io_sq *io_sq)
{
	...
	if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_HOST) {
		...
		if (!io_sq->bounce_buf_ctrl.base_buffer)
			io_sq->bounce_buf_ctrl.base_buffer = reinterpret_cast<decltype(io_sq->bounce_buf_ctrl.base_buffer)>(ENA_MEM_ALLOC(ena_dev->dmadev, size));
...
static int ena_com_indirect_table_allocate(struct ena_com_dev *ena_dev)
{
	...
	rss->host_rss_ind_tbl =
		reinterpret_cast<decltype(rss->host_rss_ind_tbl)>(ENA_MEM_ALLOC(ena_dev->dmadev,
			      requested_tbl_size));
			      ...
			      enum ena_admin_hash_functions *func)
{
	...
	rss->hash_func = (ena_admin_hash_functions)ENA_FFS(get_resp.u.flow_hash_func.selected_func);
	if (rss->hash_func)
      rss->hash_func = (ena_admin_hash_functions)(rss->hash_func - 1);
	*func = rss->hash_func;

...
#include "bypass/fastt/packet_if.hh"
#include "bypass/fastt/protocol.hh"
#include "timer.hh"
...
public:
  connection_manager(bool is_client, rte_eth_dev* eth_dev, uint16_t txq, uint16_t rxq,
      ...
      : flows(kdefaultFlowTableSize), allocator(allocator), dev(eth_dev, txq, rxq),
        scheduler(&dev), pkt_if(&scheduler, sip, eth_dev), active(),
        is_client(is_client), flush_timeout(std::chrono::microseconds(1)), flush_timer(&con_timer_manager.set) {
            flush_timer.reset(flush_timeout, timertype::PERIODICAL, flush_cb, this);
  }
  ...
  connection *open_connection(const con_config &source,
    ...
    auto [it, inserted] = flows.emplace(
        ft, new connection(allocator.get(), &pkt_if, target,
  ...
  connection *accept_connection() {
    ...
    auto [con, inserted] = add_connection(ft, be16toh(ft.dport));
...
private:
  static void flush_cb(osv_timer *timer, void *arg) {
    (void)timer;
  ...
  osv_timer::tick_t flush_timeout;
  timer_manager<osv_timer> con_timer_manager;
  osv_timer flush_timer;
};

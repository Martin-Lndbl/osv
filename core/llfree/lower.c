...
static void lower_clear(lower_t *self, bool free_all) {
  ...
  for (size_t i = 0; i < child_c; i++) {
    size_t free = free_all ? LL_MIN(CHILD_N, self->frames - i * CHILD_N) : 0;
...
static void lower_recover(lower_t *self) {
  for (size_t i = 0; i < child_count(self); ++i) {
    ...
    if (child.huge) {
      ...
      atom_store(get_child(self, i), child_new(0, true, false));
      field_init(&self->fields[i]);
    } else {
      // not a Huge Page -> count free Frames and set as counter
      uint16_t counter =
          (uint16_t)(CHILD_N - field_count_ones(&self->fields[i]));
      atom_store(get_child(self, i), child_new(counter, false, false));
    }
  }
}
...
uint8_t *lower_metadata(lower_t *self) { return (uint8_t *)self->fields; }
...
static llfree_result_t get_max(lower_t *self, uint64_t frame,
  ...
  for_offsetted(idx, LLFREE_TREE_CHILDREN / 2) {
    ...
    if (atom_update(pair, old, child_set_max, allow_reclaimed)) {
      return llfree_ok(frame_from_child(current_i * 2), old.first.reclaimed);
    }
  }
...
static llfree_result_t get_huge(lower_t *self, uint64_t frame,
  ...
  for_offsetted(idx, LLFREE_TREE_CHILDREN) {
    ...
    if (atom_update(child, old, child_set_huge, allow_reclaimed)) {
      return llfree_ok(frame_from_child(current_i), old.reclaimed);
    }
  }
...
static llfree_result_t lower_get_inner(lower_t *self, uint64_t frame,
  ...
  for_offsetted(idx, LLFREE_TREE_CHILDREN) {
    child_t old;
    _Atomic(child_t) *child = get_child(self, current_i);
    if (atom_update(child, old, child_dec, order, allow_reclaimed)) {
      llfree_result_t pos =
          field_set_next(&self->fields[current_i], frame, order);
      if (llfree_is_ok(pos)) {
        return llfree_ok(frame_from_child(current_i) + pos.frame,
                         old.reclaimed);
      }
...
llfree_result_t lower_get_at(lower_t *self, uint64_t frame, size_t order) {
  ...
  if (atom_update(child, old, child_dec, order, true)) {
    ...
    llfree_result_t ret = field_toggle(field, field_index, order, false);
    if (llfree_is_ok(ret))
      return llfree_ok(frame, old.reclaimed);
...
static llfree_result_t split_huge(_Atomic(child_t) *child, bitfield_t *field) {
  ...
  if (success) {
    ...
    success = atom_cmp_exchange(child, &expected, child_new(0, false, false));
    assert(success);
  } else {
...
llfree_result_t lower_put(lower_t *self, uint64_t frame, size_t order) {
  ...
  if (order == LLFREE_MAX_ORDER) {
    child_pair_t old;
    if (atom_update((_Atomic(child_pair_t) *)child, old, child_clear_max))
      return llfree_err(LLFREE_ERR_OK);
...
size_t lower_free_at_tree(lower_t *self, uint64_t frame) {
  assert(frame >> LLFREE_CHILD_ORDER < child_count(self));
  size_t free = 0;
  size_t start = align_down(frame >> LLFREE_CHILD_ORDER, LLFREE_TREE_CHILDREN);
...
void lower_print(lower_t *self) {
  ...
  for (size_t i = 0; i < align_up(child_count(self), LLFREE_TREE_CHILDREN);
       i++) {

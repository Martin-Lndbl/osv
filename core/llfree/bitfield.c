...
llfree_result_t field_set_next(bitfield_t *field, uint64_t start_frame,
  ...
  if (num_frames <= LLFREE_ATOMIC_SIZE) {
    for_offsetted(row, FIELD_N) {
      size_t pos = 0;
      uint64_t old;
      if (atom_update(&field->rows[current_i], old, first_zeros_aligned, order,
                      &pos)) {
        return llfree_ok((current_i * LLFREE_ATOMIC_SIZE + pos), false);
      }
    }
  ...
  for_offsetted(row / entries, FIELD_N / entries) {
    ...
    if (!failed) {
      ...
      return llfree_ok(current_i * entries * LLFREE_ATOMIC_SIZE, false);
    }
  }
